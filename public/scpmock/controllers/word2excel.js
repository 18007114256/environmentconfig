...
module.exports = function word2excel() {
  return new Promise((resloves, reject) => {
    let excelUrl = path.resolve(
      __dirname,
      "../public/assets/excel/示例接口说明书.doc"
    );
    parseWordDocument(excelUrl, (resData) => {
      // 初始化数据
      function initArr(arr = []) {
        ...
        resData.forEach((item, i) => {
          if (item == "ddchmpanyActivityInf") {
            resData.splice(i, 1); // 从下标 i 开始, 删除 1 个元素
          }
        });
        resData.reverse().forEach((item) => {
          // 根据 '功能描述' 切割大数组
          if (item == "功能描述") {
            // 倒置数组顺序
      ...
      function getRealArr(arr) {
        ...
        let resArr = arr.reverse().map((item) => {
          ...
          return {
            ...
            url: url, // 接口地址
            type: type, // 接口类型
            request: transExplain(paramsObj.request), // 接口请求参数
            response: transExplain(paramsObj.response), // 接口相应参数
          };
        });
      ...
      function transExplain(params) {
        ...
        indexList.push(params.length - 1);
        let len = indexList.length - 2;
        // [ 2, 8, 12 ]
        indexList.forEach((item, index) => {
          let next = indexList[index + 1];
          if (next) {
            if ((index == len && item + 1 != next) || item + 4 < next) {
              // 3
              let nextStart = item + 1;
              // 5
              let nextEnd = index == len ? next : next - 3;
              params.forEach((pItem, pIndex) => {
                let flag = false;
                params.forEach((citem,cindex)=>{
                  if(cindex < pIndex && citem === pItem){
                    flag = true
                  }
                })
                if (pIndex > nextStart && pIndex <= nextEnd && !flag) {
                  params[nextStart] += pItem;
                  params[pIndex] = "";
                }
              });
            }
        ...
        resParams = params.filter((item) => item != "");
        return resParams;
      }
      ...
      function writeXlsx(buffer) {
        let downloadUrl = "./public/assets/excel/transformWord.xlsx";
