...
// 获取基础数据
const baseContent = JSON.parse(fs.readFileSync("./public/assets/json/content.json"));
// 所有参数描述值对象
let allDescObj = {};
...
function getNameList(paths) {
    let maxLength = 31;
    return Object.keys(paths).map((item) => item.replace("/", "").substr(item, maxLength));
}
        ...
        let obj = {
            ...
            request:
                type == "post"
                    ...
                    : item[type].parameters,
            // 响应体
            response: item[type].responses["200"].schema.$ref.replace("#/definitions/", ""),
        };
        resArr.push(obj);
    });
...
function transType(item) {
    ...
    switch (item.type) {
        ...
        case "array":
            res = item.items.$ref
                ? getRealRequest(item.items.$ref.replace("#/definitions/", ""), "jsonArray")
                : "list";
            break;
        default:
...
function transDescription(i) {
    let obj =
        allDescObj[i]["description"] !== undefined ? allDescObj[i] : { description: "暂无描述" };
